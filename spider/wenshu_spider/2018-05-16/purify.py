...
def extract_exe_date(word):
    ...
    d = re.findall(r'于(.{10}?)日立案执行', word)
    if d: return d[0] + "日"

    d = re.findall(r'于(.{10}?)日申请执行', word)
    if d: return d[0] + "日"

    d = re.findall(r'于(.{10}?)日申请强制执行', word)
    if d: return d[0] + "日"
...
def extract_hear_date(word):
    ...
    d = re.findall(r'于(.{10}?)日公开开庭', word)
    if d:
        return d[0] + "日"
    else:
        return ""
...
def extract_exe_rule(word):
    """ 执行裁定 """

    d = re.findall(r'裁定如下(.*?)。', word)
    if d: return d[0] + "。"

    d = re.findall(r'判决如下(.*?)。', word)
    if d: return d[0] + "。"

    return ""


def extract_exe_ref_case_no(word):
    """ 获取 执行依据的案号 """

    d = re.findall(r'作出的(.*?)号', word)
    if d:
        return d[0] + "号"
    else:
        return ""
...
def main():
    ...
    win_over_string = "本案受理费13980元予以退回。审判长　　徐新卫审判员　　车兴民审判员　　雷晓宁二〇一八年一月××日书记员"

    hear_string = "本院于2017年9月7日立案受理。审理中原告申请撤回了对朱建明的起诉，本院裁定予以准许。后本院依法组成合议庭于2017年12月18日公开开庭了进行审理。"
    ...
    # print(extract_hear_date(hear_string))

    # loan_principal = extract_loan_principal(c)
    # print(loan_principal)

    print(extract_win_over(win_over_string))
